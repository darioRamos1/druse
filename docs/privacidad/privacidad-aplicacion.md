...
- **P-04 — Implementado en el código; distribución pendiente.** Preferencias → Privacidad muestra el aviso sin cargar documentos externos. La landing enlaza [Privacidad de la aplicación](../../landing/privacidad-aplicacion.html). Ambos usan el mismo contenido de `frontend/src/app/shared/privacy/privacy-notice.html`; regenerar la página con `node build/scripts/generar-privacidad-web.cjs` al editarlo. Pages comprueba la sincronización antes de publicar. Falta distribuir una versión que incluya el cambio y confirmar el enlace público. El texto sigue provisional por P-02, P-03 y P-05.
