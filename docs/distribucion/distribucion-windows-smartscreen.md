...
`build/scripts/package.ps1` ya genera un instalador NSIS para Windows x64 con la API autocontenida y el frontend. La versión configurada es 1.2.1. La tarea pendiente del plan es **REL-005: firma Authenticode y sellado de tiempo**.
...
Resultado: `shells/desktop-tauri/target/release/bundle/nsis/Druse_1.2.1_x64-setup-completo.exe`, acompañado de su `.sig`. La variante sin Informix se genera añadiendo `-WithoutInformix`.
