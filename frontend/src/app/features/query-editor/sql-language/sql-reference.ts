...
const INFORMIX: readonly DatabaseEngine[] = ['informix', 'informixsqli'];
...
const ALL: readonly SqlReferenceEntry[] = [...FUNCTIONS, ...KEYWORDS];
...
export function referenceFor(name: string, engine?: DatabaseEngine): SqlReferenceEntry | undefined {
  ...
  return ALL.find((entry) => entry.name.toUpperCase() === key && available(entry, engine));
}
