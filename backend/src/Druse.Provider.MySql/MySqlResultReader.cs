...
/// <summary>
/// Lectura progresiva sobre un <see cref="DbDataReader"/> de MySqlConnector.
///
...
/// </summary>
internal sealed class MySqlResultReader : IQueryResultReader
{
    private readonly DbCommand _command;
    private readonly DbDataReader _reader;
    private readonly string?[] _buffer;

    private MySqlResultReader(
        DbCommand command,
        DbDataReader reader,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new MySqlResultReader(command, reader, columns);
        }
        ...
        while (await _reader.ReadAsync(cancellationToken))
        {
            for (var ordinal = 0; ordinal < _buffer.Length; ordinal++)
            {
                _buffer[ordinal] = await _reader.IsDBNullAsync(ordinal, cancellationToken)
                    ? null
                    : MySqlValueFormatter.Format(_reader.GetValue(ordinal));
            }
