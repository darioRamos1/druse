...
public sealed class TransferTracker : ITransferTracker, IDisposable
{
    ...
    /// </summary>
    private static TransferProgress Newer(TransferProgress previous, TransferProgress candidate)
    {
        var terminado = previous.Outcome != TransferOutcome.Running;

        if (terminado)
        {
            return candidate.Outcome == TransferOutcome.Running ? previous : candidate;
        }

        return candidate.Outcome == TransferOutcome.Running && candidate.Elapsed < previous.Elapsed
            ? previous
            : candidate;
    }
