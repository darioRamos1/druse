...
/// <summary>
/// Lectura progresiva sobre un <see cref="DbDataReader"/> de Npgsql.
///
...
/// </summary>
internal sealed class PostgreSqlResultReader : IQueryResultReader
{
    private readonly DbCommand _command;
    private readonly DbDataReader _reader;
    private readonly string?[] _buffer;

    private PostgreSqlResultReader(
        DbCommand command,
        DbDataReader reader,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // SequentialAccess permite ir liberando cada fila según se lee, en
            // lugar de tener el registro entero en memoria.
        ...
        while (await _reader.ReadAsync(cancellationToken))
        {
            for (var ordinal = 0; ordinal < _buffer.Length; ordinal++)
            {
                _buffer[ordinal] = await _reader.IsDBNullAsync(ordinal, cancellationToken)
                    ? null
                    : PostgreSqlValueFormatter.Format(_reader.GetValue(ordinal));
            }
